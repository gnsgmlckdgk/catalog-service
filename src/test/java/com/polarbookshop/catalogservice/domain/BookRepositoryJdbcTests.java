...
public class BookRepositoryJdbcTests {
    ...
    void findBookIsbnWhenExisting() {
        ...
        jdbcAggregateTemplate.insert(book); // JdbcAggregateTemplate는 테스트에 필요한 데이터를 준비하는 데 사용한다.
        Optional<Book> actualBook = bookRepository.findByIsbn(bookIsbn);

        assertThat(actualBook).isPresent(); // Optional 객체가 값을 가지고 있는지 체크(isPresent())
